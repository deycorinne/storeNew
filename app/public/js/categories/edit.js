 ...
 * edit category validation code- JavaScript
 *
 */
...
function formValidation() {
    ...
    var logo = document.getElementById('new_logo').value;
...
function logo_validation(l) {
    ...
    var delete_logo = document.getElementById('delete_logo');
    var l_len = l.length;

    // if checkbox is checked, we need a new image to replace it
    if(delete_logo.checked){
        if (l_len === 0) {
            alert("If you delete the logo, you must upload a new one!");
            return false;
        } else {
            return true;
        }
